...
def app_created_lastly(dir, filetype):         #传入的dir就是文件夹A的路径，如果你的脚本就在当前路径，直接传入“.”即可
    ...
    for i in os.listdir(dir+os.sep+dir_created_lastly):                         #找到该文件路径下面的IPA文件
        if filetype == 'ipa':
            ...
            if i.find('.apk') != -1:
                apk_path = dir+os.sep+dir_created_lastly+os.sep+i
                return apk_path
...
if __name__ == '__main__':
    app_dir = '.'
    IpaPath = app_created_lastly(app_dir, 'apk')
    upload_app(IpaPath)